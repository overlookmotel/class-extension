...
module.exports = function extend(extension, options) {
	...
	// Init cache
	let cache = subclassCache.get(Class);
	if (!cache) {
		cache = new WeakMap();
		subclassCache.set(Class, cache);
	}
	...
	// Not in cache - extend
	const SubClass = extension.extend(Class, extension);
	...
	if (name) {
		const newNamedExtensions = Object.assign(Object.create(null), namedExtensions);
		newNamedExtensions[name] = extension;
		SubClass[NAMED_EXTENSIONS] = newNamedExtensions;
	}
